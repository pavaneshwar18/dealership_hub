...
type AdminBranchAnalyticsClientProps = {
...
export function AdminBranchAnalyticsClient({ initialSales }: AdminBranchAnalyticsClientProps) {
  ...
  const filteredSales = useMemo(() => {
    return initialSales.filter((s) => {
      // For Branch Analytics, we include all sales (cash + finance)
  ...
  const handleExportCSV = () => {
    const headers = ["Branch Name", "Units Sold", "Share (%)", "Total Revenue (INR)", "Avg per Vehicle (INR)"];
    ...
    link.setAttribute("download", `branch_analytics_${selectedPeriod.toLowerCase().replace(/[\s-]/g, '_')}.csv`);
        ...
        <body>
          ...
          <div class="charts-grid">
            <div class="card">
              ...
              <p class="card-sub">Branch volume share</p>
            ...
            <div class="card">
              <h3 class="card-title">Sales Volume by Branch</h3>
    ...
    <div className="space-y-6">
      ...
      {stats.length > 0 ? (
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col justify-between">
            <div>
              <h2 className="text-base font-bold text-slate-900">Sales volume by Branch</h2>
              <p className="text-xs text-slate-500 mt-0.5">Total units compared across branches</p>
            </div>
