...
export default async function BackOfficeSaleDetailPage({
  ...
  return (
    <>
      ...
      <section className="mb-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="mb-4 text-lg font-semibold text-slate-900">Post-Sale Operations</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-xl bg-slate-50 px-4 py-3 border border-slate-100">
            <p className="text-xs font-medium uppercase tracking-wide text-slate-500">TR Status</p>
            {sale.trCompleted ? (
              <div className="mt-1">
                <p className="text-sm font-semibold text-slate-900">{sale.trNumber}</p>
                <p className="text-xs text-slate-500">{sale.trDate ? formatDate(sale.trDate) : ""}</p>
              </div>
            ) : (
          ...
          <div className="rounded-xl bg-slate-50 px-4 py-3 border border-slate-100">
            <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Invoice ID</p>
            {sale.invoiceCompleted ? (
              <p className="mt-1 text-sm font-semibold text-slate-900">{sale.invoiceId}</p>
            ) : (
              <p className="mt-1 text-sm font-medium text-amber-600">Pending</p>
            )}
          </div>
          <div className="rounded-xl bg-slate-50 px-4 py-3 border border-slate-100">
            <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Insurance</p>
            {sale.insuranceCompleted ? (
              <div className="mt-1">
                <p className="text-sm font-semibold text-slate-900">{sale.insuranceProvider}</p>
                <p className="text-xs text-slate-500">Pol: {sale.insurancePolicyNumber}</p>
              </div>
            ) : (
              <p className="mt-1 text-sm font-medium text-amber-600">Pending</p>
            )}
          ...
          <div className="rounded-xl bg-slate-50 px-4 py-3 border border-slate-100">
            <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Permanent Plate</p>
            {sale.numberPlateCompleted ? (
              <p className="mt-1 text-sm font-semibold text-slate-900">{sale.permanentNumberPlate}</p>
            ) : (
              <p className="mt-1 text-sm font-medium text-amber-600">Pending</p>
            )}
          </div>
